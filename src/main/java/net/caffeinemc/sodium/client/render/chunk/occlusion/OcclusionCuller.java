package net.caffeinemc.sodium.client.render.chunk.occlusion;
...
import net.caffeinemc.sodium.client.util.collections.DoubleBufferedQueue;
import net.caffeinemc.sodium.client.util.collections.ReadQueue;
import net.caffeinemc.sodium.client.util.collections.WriteQueue;
...
public class OcclusionCuller {
    ...
    private static int getOutwardDirections(SectionPos origin,
                                            RenderSection section) {
        var planes = 0;
    ...
    // SodiumExtra
    private static boolean isWithinRenderDistance(CameraTransform camera,
                                                  RenderSection section,
                                                  float maxDistance) {
        ...
        if (fogDistance == 33) {
            val ox = section.getOriginX() - camera.intX;
            val oz = section.getOriginZ() - camera.intZ;
            val dx = nearestToZero(ox, ox + 16) - camera.fracX;
            val dz = nearestToZero(oz, oz + 16) - camera.fracZ;
        ...
        val ox = section.getOriginX() - camera.intX;
        val oy = section.getOriginY() - camera.intY;
        val oz = section.getOriginZ() - camera.intZ;
        ...
        val dx = nearestToZero(ox - 1, ox + 17) - camera.fracX;
        val dy = nearestToZero(oy - 1, oy + 17) - camera.fracY;
        val dz = nearestToZero(oz - 1, oz + 17) - camera.fracZ;
