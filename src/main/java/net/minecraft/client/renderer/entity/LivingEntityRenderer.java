...
public abstract class LivingEntityRenderer<T extends LivingEntity, S extends LivingEntityRenderState, M extends EntityModel<? super S>>
    ...
    public void render(S livingEntityRenderState,
                       PoseStack poseStack,
        ...
        if (livingEntityRenderState instanceof ArmorStandRenderState && !SodiumExtraClientMod.options().renderSettings.armorStand) {
            if (livingEntityRenderState.nameTag != null) {
                renderNameTag(livingEntityRenderState, livingEntityRenderState.nameTag, poseStack, multiBufferSource, i1);
            }
        ...
        float f1 = livingEntityRenderState.scale;
        poseStack.scale(f1, f1, f1);
        this.setupRotations(livingEntityRenderState, poseStack, livingEntityRenderState.bodyRot, f1);
        poseStack.scale(-1.0F, -1.0F, 1.0F);
        this.scale(livingEntityRenderState, poseStack);
        poseStack.translate(0.0F, -1.501F, 0.0F);
        this.model.setupAnim(livingEntityRenderState);
        boolean flag1 = this.isBodyVisible(livingEntityRenderState);
        boolean flag = !flag1 && !livingEntityRenderState.isInvisibleToPlayer;
        RenderType rendertype = this.getRenderType(livingEntityRenderState, flag1, flag, livingEntityRenderState.appearsGlowing);
        if (rendertype != null) {
            ...
            this.model.renderToBuffer(poseStack, vertexconsumer, i1, i, k);
        }

        if (this.shouldRenderLayers(livingEntityRenderState)) {
            for (RenderLayer<S, M> renderlayer : this.layers) {
                renderlayer.render(poseStack, multiBufferSource, i1, livingEntityRenderState, livingEntityRenderState.yRot, livingEntityRenderState.xRot);
            }
        }

        poseStack.popPose();
        super.render(livingEntityRenderState, poseStack, multiBufferSource, i1);
    }
    ...
    protected boolean shouldShowName(T entityLiving, double d) {
        // SodiumExtra
        if (entityLiving instanceof AbstractClientPlayer && !SodiumExtraClientMod.options().renderSettings.playerNameTag) {
            return false;
        }

        if (entityLiving.isDiscrete()) {
            float f = 32.0F;
            if (d >= 1024.0) {
                return false;
            }
        ...
        boolean flag = !entityLiving.isInvisibleTo(localplayer);
        if (entityLiving != localplayer) {
        ...
        return Minecraft.renderNames() && entityLiving != minecraft.getCameraEntity() && flag && !entityLiving.isVehicle();
    }
