...
public abstract class EntityRenderer<T extends Entity, S extends EntityRenderState> {
    ...
    public boolean shouldRender(T entityArg, Frustum instance, double p_114493_, double p_114494_, double p_114495_) {
        if (!entityArg.shouldRender(p_114493_, p_114494_, p_114495_)) {
            return false;
        }

        if (!this.affectedByCulling(entityArg)) {
            return true;
        }

        AABB aabb = this.getBoundingBoxForCulling(entityArg).inflate(0.5);
        ...
        if (renderer == null) {
            visible = instance.isVisible(aabb);
        } else {
            visible = renderer.isEntityVisible(this, entityArg) && instance.isVisible(aabb);
        }
        ...
        if (entityArg instanceof Leashable leashable) {
            Entity entity = leashable.getLeashHolder();
            if (entity != null) {
                return instance.isVisible(this.entityRenderDispatcher.getRenderer(entity).getBoundingBoxForCulling(entity));
            }
        }
