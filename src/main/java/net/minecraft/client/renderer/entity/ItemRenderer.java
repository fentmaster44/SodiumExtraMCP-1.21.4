...
import net.caffeinemc.sodium.client.render.immediate.model.BakedModelEncoder;
...
public class ItemRenderer {
    ...
    private static void renderModelLists(BakedModel bakedModel,
                                         int[] colors,
                                         int light,
                                         int overlay,
                                         PoseStack poseStack,
        ...
        // Sodium
        val randomsource = random.get();
        var writer = VertexConsumerUtils.convertOrLog(vertexConsumer);

        for (val direction : Direction.VALUES) {
            randomsource.setSeed(42L);

            val quads = bakedModel.getQuads(null, direction, randomsource);

            if (writer == null) {
                renderQuadList(poseStack, vertexConsumer, quads, colors, light, overlay);
                continue;
            }
        ...
        val quads = bakedModel.getQuads(null, null, randomsource);
    ...
    private static void renderBakedItemQuads(PoseStack.Pose matrices,
