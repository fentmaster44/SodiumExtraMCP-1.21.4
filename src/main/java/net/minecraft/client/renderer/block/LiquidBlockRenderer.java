...
public class LiquidBlockRenderer {
    ...
    public void tessellate(BlockAndTintGetter view,
                           BlockPos pos,
        ...
        // Fabric
        val info = FluidRenderingImpl.getCurrentInfo();

        if (info.handler == null) {
            val handler = FluidRenderHandlerRegistry.INSTANCE.get(fluidState.getType());
        ...
        TextureAtlasSprite[] sprites = flag ? this.lavaIcons : this.waterIcons;
        int i = flag ? 16777215 : BiomeColors.getAverageWaterColor(view, pos);

        // Fabric
        int color = modTintColor(info, 16, view, pos, vertexConsumer, blockState, fluidState);
        float f = (float)(i >> color & 0xFF) / 255.0F;

        float f1 = (float)(i >> 8 & 0xFF) / 255.0F;
        float f2 = (float)(i & 0xFF) / 255.0F;
        BlockState blockstate = view.getBlockState(pos.relative(Direction.DOWN));
        FluidState fluidstate = blockstate.getFluidState();
        BlockState blockstate1 = view.getBlockState(pos.relative(Direction.UP));
        FluidState fluidstate1 = blockstate1.getFluidState();
        BlockState blockstate2 = view.getBlockState(pos.relative(Direction.NORTH));
        FluidState fluidstate2 = blockstate2.getFluidState();
        BlockState blockstate3 = view.getBlockState(pos.relative(Direction.SOUTH));
        FluidState fluidstate3 = blockstate3.getFluidState();
        BlockState blockstate4 = view.getBlockState(pos.relative(Direction.WEST));
        FluidState fluidstate4 = blockstate4.getFluidState();
        BlockState blockstate5 = view.getBlockState(pos.relative(Direction.EAST));
        FluidState fluidstate5 = blockstate5.getFluidState();

        boolean isLava = modLavaCheck(info, !isNeighborSameFluid(fluidState, fluidstate1));
        sprites = modSpriteArray(info, sprites);

        boolean flag2 = shouldRenderFace(fluidState, blockState, Direction.DOWN, fluidstate) && !isFaceOccludedByNeighbor(Direction.DOWN, 0.8888889F, blockstate);
        boolean flag3 = shouldRenderFace(fluidState, blockState, Direction.NORTH, fluidstate2);
        boolean flag4 = shouldRenderFace(fluidState, blockState, Direction.SOUTH, fluidstate3);
        boolean flag5 = shouldRenderFace(fluidState, blockState, Direction.WEST, fluidstate4);
        boolean flag6 = shouldRenderFace(fluidState, blockState, Direction.EAST, fluidstate5);
        if (isLava || flag2 || flag6 || flag5 || flag3 || flag4) {
            float f3 = view.getShade(Direction.DOWN, true);
            float f4 = view.getShade(Direction.UP, true);
            float f5 = view.getShade(Direction.NORTH, true);
            float f6 = view.getShade(Direction.WEST, true);
            Fluid fluid = fluidState.getType();
            float f11 = this.getHeight(view, fluid, pos, blockState, fluidState);
            float f7;
            float f8;
            float f9;
            float f10;
            if (f11 >= 1.0F) {
                ...
                float f12 = this.getHeight(view, fluid, pos.north(), blockstate2, fluidstate2);
                float f13 = this.getHeight(view, fluid, pos.south(), blockstate3, fluidstate3);
                float f14 = this.getHeight(view, fluid, pos.east(), blockstate5, fluidstate5);
                float f15 = this.getHeight(view, fluid, pos.west(), blockstate4, fluidstate4);
                f7 = this.calculateAverageHeight(view, fluid, f11, f12, f14, pos.relative(Direction.NORTH).relative(Direction.EAST));
                f8 = this.calculateAverageHeight(view, fluid, f11, f12, f15, pos.relative(Direction.NORTH).relative(Direction.WEST));
                f9 = this.calculateAverageHeight(view, fluid, f11, f13, f14, pos.relative(Direction.SOUTH).relative(Direction.EAST));
                f10 = this.calculateAverageHeight(view, fluid, f11, f13, f15, pos.relative(Direction.SOUTH).relative(Direction.WEST));
            }
            ...
            if (isLava && !isFaceOccludedByNeighbor(Direction.UP, Math.min(Math.min(f8, f10), Math.min(f9, f7)), blockstate1)) {
                f8 -= 0.001F;
                f10 -= 0.001F;
                f9 -= 0.001F;
                f7 -= 0.001F;
                Vec3 vec3 = fluidState.getFlow(view, pos);
                float f17;
                float f18;
                float f19;
                float f20;
                ...
                int l = this.getLightColor(view, pos);
                float f57 = f4 * f;
                float f29 = f4 * f1;
                float f30 = f4 * f2;
                this.vertex(vertexConsumer, f36 + 0.0F, f37 + f8, f38 + 0.0F, f57, f29, f30, f17, f21, l);
                this.vertex(vertexConsumer, f36 + 0.0F, f37 + f10, f38 + 1.0F, f57, f29, f30, f18, f22, l);
                this.vertex(vertexConsumer, f36 + 1.0F, f37 + f9, f38 + 1.0F, f57, f29, f30, f19, f23, l);
                this.vertex(vertexConsumer, f36 + 1.0F, f37 + f7, f38 + 0.0F, f57, f29, f30, f20, f24, l);
                if (fluidState.shouldRenderBackwardUpFace(view, pos.above())) {
                    this.vertex(vertexConsumer, f36 + 0.0F, f37 + f8, f38 + 0.0F, f57, f29, f30, f17, f21, l);
                    this.vertex(vertexConsumer, f36 + 1.0F, f37 + f7, f38 + 0.0F, f57, f29, f30, f20, f24, l);
                    this.vertex(vertexConsumer, f36 + 1.0F, f37 + f9, f38 + 1.0F, f57, f29, f30, f19, f23, l);
                    this.vertex(vertexConsumer, f36 + 0.0F, f37 + f10, f38 + 1.0F, f57, f29, f30, f18, f22, l);
            ...
            if (flag2) {
                ...
                int k = this.getLightColor(view, pos.below());
                float f46 = f3 * f;
                float f48 = f3 * f1;
                float f50 = f3 * f2;
                this.vertex(vertexConsumer, f36, f37 + f16, f38 + 1.0F, f46, f48, f50, f40, f43, k);
                this.vertex(vertexConsumer, f36, f37 + f16, f38, f46, f48, f50, f40, f42, k);
                this.vertex(vertexConsumer, f36 + 1.0F, f37 + f16, f38, f46, f48, f50, f41, f42, k);
                this.vertex(vertexConsumer, f36 + 1.0F, f37 + f16, f38 + 1.0F, f46, f48, f50, f41, f43, k);
            }

            int j = this.getLightColor(view, pos);
            ...
            for (Direction direction : Direction.Plane.HORIZONTAL) {
                ...
                if (flag7 && !isFaceOccludedByNeighbor(direction, Math.max(f44, f45), view.getBlockState(pos.relative(direction)))) {
                    ...
                    if (!flag) {
                        // Fabric
                        sprite2 = modSideSpriteForOverlay(info, sprite2, view.getBlockState(relativePos).getBlock());
                    }

                    float f56 = sprite2.getU(0.0F);
                    float f58 = sprite2.getU(0.5F);
                    float f59 = sprite2.getV((1.0F - f44) * 0.5F);
                    float f60 = sprite2.getV((1.0F - f45) * 0.5F);
                    float f31 = sprite2.getV(0.5F);
                    float f32 = direction.getAxis() == Direction.Axis.Z ? f5 : f6;
                    float f33 = f4 * f32 * f;
                    float f34 = f4 * f32 * f1;
                    float f35 = f4 * f32 * f2;

                    this.vertex(vertexConsumer, f47, f37 + f44, f49, f33, f34, f35, f56, f59, j);
                    this.vertex(vertexConsumer, f51, f37 + f45, f52, f33, f34, f35, f58, f60, j);
                    this.vertex(vertexConsumer, f51, f37 + f16, f52, f33, f34, f35, f58, f31, j);
                    this.vertex(vertexConsumer, f47, f37 + f16, f49, f33, f34, f35, f56, f31, j);

                    if (sprite2 != modWaterOverlaySprite(info)) {
                        this.vertex(vertexConsumer, f47, f37 + f16, f49, f33, f34, f35, f56, f31, j);
                        this.vertex(vertexConsumer, f51, f37 + f16, f52, f33, f34, f35, f58, f31, j);
                        this.vertex(vertexConsumer, f51, f37 + f45, f52, f33, f34, f35, f58, f60, j);
                        this.vertex(vertexConsumer, f47, f37 + f44, f49, f33, f34, f35, f56, f59, j);
    ...
    // Fabric
    public TextureAtlasSprite modSideSpriteForOverlay(FluidRenderHandlerInfo info,
                                                      TextureAtlasSprite chk,
                                                      Block block) {
    ...
    // Fabric
    private boolean modLavaCheck(FluidRenderHandlerInfo info, boolean chk) {
    ...
    // Fabric
    private TextureAtlasSprite[] modSpriteArray(FluidRenderHandlerInfo info,
                                                TextureAtlasSprite[] chk) {
        return info.handler != null ? info.sprites : chk;
    }

    // Fabric
    public TextureAtlasSprite modWaterOverlaySprite(FluidRenderHandlerInfo info) {
        return info.handler != null && info.hasOverlay ? info.overlaySprite : waterOverlay;
    }

    public int modTintColor(FluidRenderHandlerInfo info,
                            int chk,
