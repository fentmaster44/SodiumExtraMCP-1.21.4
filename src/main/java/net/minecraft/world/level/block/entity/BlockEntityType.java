...
public class BlockEntityType<T extends BlockEntity> {
    ...
    // Sodium
    @Getter private BlockEntityRenderPredicate[] renderPredicates = new BlockEntityRenderPredicate[0];
    ...
    public void addRenderPredicate(BlockEntityRenderPredicate predicate) {
        renderPredicates = ArrayUtils.add(renderPredicates, predicate);
    }

    public boolean removeRenderPredicate(BlockEntityRenderPredicate predicate) {
        int index = ArrayUtils.indexOf(renderPredicates, predicate);

        if (index == ArrayUtils.INDEX_NOT_FOUND) {
            return false;
        }

        renderPredicates = ArrayUtils.remove(renderPredicates, index);
        return true;
    }
